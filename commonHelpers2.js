import"./assets/sidebar-e534f703.js";import{a as H}from"./assets/vendor-169f9f98.js";const p="data:image/png;base64,",h="data:image/png;base64,",k="data:image/png;base64,",g=document.querySelector(".slist-card-section"),u=document.querySelector(".slist-demo-thumb"),c="local-storage-books";let n=[];function b(s){try{const t=localStorage.getItem(s),i=JSON.parse(t);return console.log(i),n=i.map(A=>A.dataId),console.log(n),n}catch{console.log("error"),r()}}const f=b(c);console.log(n);D(f);async function D(s){let t=[];if(s===null||s.length===0||!c)r();else{for(const A of s){const l=await Q(A);t.push(l);const a=l.buy_links.find(e=>e.name==="Amazon");console.log(a.url)}const i=U(t);g.innerHTML=i,g.addEventListener("click",y)}}function v({_id:s,book_image:t,title:i,list_name:A,description:l,author:a,buy_links:e}){let m=e.map(o=>o).find(o=>o.name==="Amazon"),d=e.map(o=>o).find(o=>o.name==="Apple Books");if(localStorage.getItem("theme")==="dark"){const o=k;return`<div class="slist-card-list">
            <li class="slist-card-item">
              <button type="button" data-id=${s} class="slist-del-btn js-slist-del-btn">
              </button>
              <div class="slist-card-picture">
                <img src="${t}" class="slist-book-img" alt="books" />
              </div>
              <div class="slist-info-container">
                <h3 class="slist-book-header">${i}</h3>
                <h4 class="slist-book-category">${A}</h4>
                <p class="slist-book-description">${l}</p>
                <h5 class="slist-book-autor">${a}</h5>

                <div class="slist-nav">
                  <ul class="slist-nav-list">
                    <li class="slist-nav-item">
                      <a href="${m.url}" class="slist-nav-link">

                      <img class="img-amazone logo" src="${o}" alt="app">
                     </a>

                    </li>

                    <li class="slist-nav-item">
                      <a href="${d.url}" class="slist-nav-link"><img class="img-app logo" src="${h}" alt="app"></a>
                      
                    </li>
                  </ul>
                </div>
              </div>
            </li>
          </div>`}else return`<div class="slist-card-list">
            <li class="slist-card-item">
              <button type="button" data-id=${s} class="slist-del-btn js-slist-del-btn">
              </button>
              <div class="slist-card-picture">
                <img src="${t}" class="slist-book-img" alt="books" />
              </div>
              <div class="slist-info-container">
                <h3 class="slist-book-header">${i}</h3>
                <h4 class="slist-book-category">${A}</h4>
                <p class="slist-book-description">${l}</p>
                <h5 class="slist-book-autor">${a}</h5>

                <div class="slist-nav">
                  <ul class="slist-nav-list">
                    <li class="slist-nav-item">
                      <a href="${m.url}" class="slist-nav-link">

                      <img class="img-amazone logo" src="${p}" alt="app">
                     </a>

                    </li>

                    <li class="slist-nav-item">
                      <a href="${d.url}" class="slist-nav-link"><img class="img-app logo" src="${h}" alt="app"></a>
                      
                    </li>
                  </ul>
                </div>
              </div>
            </li>
          </div>`}function U(s){return s.map(v).join("")}function y(s){if(s.target.nodeName!=="BUTTON")return;const t=s.target.closest("li");console.log(t);const i=s.target.dataset.id;console.log(i),t.remove();const A=localStorage.getItem(c);let l=JSON.parse(A);console.log(l),l.splice(l.findIndex(e=>e.dataId===i),1),console.log(l),l.length===0&&r();const a=JSON.stringify(l);localStorage.setItem(c,a)}async function Q(s){const t=`https://books-backend.p.goit.global/books/${s}`;try{return(await H.get(t)).data}catch{console.log("Результатів не знайдено.")}}function r(){u.classList.remove("is-hidden")}
//# sourceMappingURL=commonHelpers2.js.map
